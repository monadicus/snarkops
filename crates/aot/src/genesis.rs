...
pub fn genesis_quorum<R: Rng + CryptoRng>(
    ...
    let solutions = Solutions::<Network>::from(None); // The genesis block does not require solutions.
    ...
    //     .map(|_| vm.execute(private_key, locator, inputs.iter(), None, 0, None,
    // rng))     .collect::<Result<Vec<_>, _>>()?;
    ...
    pub fn parse(self) -> Result<()> {
        ...
        let accounts = (0..self.additional_accounts)
            .map(|_| {
                ...
                public_balances.insert(addr, self.additional_accounts_balance * 2);
