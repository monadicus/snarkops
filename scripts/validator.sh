...
# spin up a validator using committee member <arg 1>'s key
...
cp -r $LEDGER $LEDGER"_$1"

$SNARKOS start --nodisplay --validator --nocdn \
  --rest-rps 1000 \
  --verbosity 4 \
  --bft "0.0.0.0:500$1" \
  --node "0.0.0.0:413$1" \
  --rest "0.0.0.0:303$1" \
  --genesis $GENESIS \
  --storage_path $LEDGER"_$1" \
  --private-key $(pk $1) \
  --validators "127.0.0.1:5000,127.0.0.1:5001,127.0.0.1:5002,127.0.0.1:5003,127.0.0.1:5004"
